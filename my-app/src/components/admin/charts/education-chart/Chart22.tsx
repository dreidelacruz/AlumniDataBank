...
function Chart22({selectedYear}: any) {
    ...
    const seminarData = seminarScores.map(
      (seminar) =>
        filteredYearData.educationalDetail.seminar[
          seminar
        ]
    );
    const total = seminarData.reduce((acc, val) => acc + val, 0);
    const seminarPercentages = seminarData.map((data) =>

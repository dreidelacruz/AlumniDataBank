...
function Chart18({selectedYear}: any) {
    ...
    const gedData = gedScores.map(
      (ged) =>
        filteredYearData.educationalDetail.ged[
          ged
        ]
    );
    const total = gedData.reduce((acc, val) => acc + val, 0);
    const gedPercentages = gedData.map((data) =>

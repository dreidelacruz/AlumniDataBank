...
function Chart23({selectedYear}: any) {
    ...
    const ojtData = ojtScores.map(
      (ojt) =>
        filteredYearData.educationalDetail.ojt[
          ojt
        ]
    );
    const total = ojtData.reduce((acc, val) => acc + val, 0);
    const ojtPercentages = ojtData.map((data) =>
